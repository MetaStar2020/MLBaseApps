...
class SpeechRecognizer {
    ...
    } // Note: Selectors are normally used to enable using an Objective-C method..
    ...
    func startRecording(completion: @escaping (String?) -> ()) {
        ...
        inputNode.installTap(onBus: inputBus, bufferSize: 1024,
                             format: recordingFormat) {
            (buffer: AVAudioPCMBuffer, when: AVAudioTime) in self.request?.append(buffer)
            }
